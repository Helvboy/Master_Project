...
path_tool = os.path.join(path_code, "Toolbox")
sys.path.append(os.path.abspath(path_code))
sys.path.append(os.path.abspath(path_tool))
...
#------------------
THRESHOLD    = -80  # in dBm
TIME_TOO_OLD = 3    # in seconds
TIME_1_PASS  = 7    # in seconds
...
def state_update( EPCs, Tags, read_tags, Counter ):
    ...
    # delete old tags and update counter
    for i in ind[::-1]: # supp in a reverse a way to not generate problem of index
        modif = (Tags[i][2] - Tags[i][1])
        Counter += modif                                              # add variable counting incoming and outcoming person
        Tags.pop(i)
        EPCs.pop(i)
        comm_serv.communicate_staff_flow(-modif)
...
def displayer_run(Counter=0, read_tags=[], Tags=[]):
    """
...
def thread_reading( ):
    ...
    while Flags.Thread:
        ...
        if Flags.Run:
            ...
            displayer_run( Counter, read_tags_u, Tags )
            print("\n\n", read_tags)                                                                                # temp
        ...
        if Flags.Request[:3] == "exp":  # Export data special case
